...
{
  auto ptr = bit::core::exclusive_ptr<::base>{};
...
{
  bit::core::exclusive_ptr<::base> ptr = nullptr;
...
{
  auto p1 = bit::core::make_exclusive<::derived>();
  auto p1_old = p1.get();
  auto p2 = std::move(p1);
...
TEST_CASE("exclusive_ptr<T>::exclusive_ptr( exclusive_ptr<U>&& )")
{
  auto p1 = bit::core::make_exclusive<::derived>();
  auto p1_old = p1.get();
  auto p2 = bit::core::exclusive_ptr<::base>(std::move(p1));
...
{
  auto p = bit::core::make_exclusive<::derived>();
  auto p_old = p.get();

  p = bit::core::make_exclusive<::derived>();
...
{
  auto p = bit::core::make_exclusive<::base>();
  auto p_old = p.get();
...
TEST_CASE("casts::const_pointer_cast<T>( exclusive_ptr<U>&& )")
{
  ...
  auto p1 = bit::core::make_exclusive<::derived>();
  auto p1_old = p1.get();
  auto p2 = bit::core::exclusive_ptr<const ::derived>(std::move(p1));
...
TEST_CASE("casts::reinterpret_pointer_cast<T>( exclusive_ptr<U>&& )")
{
  ...
  auto p1 = bit::core::make_exclusive<::derived>();
  auto p1_old = p1.get();
  auto p2 = bit::core::exclusive_ptr<void>(std::move(p1));
