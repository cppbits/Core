...
TEST_CASE("compressed_pair( T1&&, T2&& )")
{
  SECTION("T1 and T2 are the same classification")
  {
    ...
    {
      auto pair = bit::core::make_compressed_pair(empty_non_final<0>{},empty_non_final<1>{});

      REQUIRE( sizeof(pair) == sizeof(empty_non_final<0>) );
    }
    SECTION("T1 & T2 are non-empty, non-final")
    {
      auto pair = bit::core::make_compressed_pair(non_empty_non_final<0>{},non_empty_non_final<1>{});

      REQUIRE( sizeof(pair) >= sizeof(non_empty_non_final<0>)*2 );
    }
    SECTION("T1 & T2 are empty, final")
    {
      auto pair = bit::core::make_compressed_pair(empty_final<0>{},empty_final<1>{});

      REQUIRE( sizeof(pair) >= sizeof(empty_final<0>)*2 );
    }
    SECTION("T1 & T2 are non-empty, final")
    {
      auto pair = bit::core::make_compressed_pair(non_empty_final<0>{},non_empty_final<1>{});
  ...
  SECTION("T1 and T2 are different classifications")
  {
    ...
    {
      auto pair = bit::core::make_compressed_pair(empty_non_final<0>{},non_empty_non_final<0>{});

      REQUIRE( sizeof(pair) == sizeof(non_empty_non_final<0>) );
    }
    SECTION("T1 is non-empty, non-final & T2 is empty, non-final")
    {
      auto pair = bit::core::make_compressed_pair(non_empty_non_final<0>{},empty_non_final<0>{});
    ...
    {
      auto pair = bit::core::make_compressed_pair(empty_final<0>{},non_empty_non_final<0>{});

      REQUIRE( sizeof(pair) >= sizeof(non_empty_non_final<0>) );
    }
    SECTION("T1 is non-empty, non-final & T2 is empty, final")
    {
      auto pair = bit::core::make_compressed_pair(non_empty_non_final<0>{},empty_final<0>{});
    ...
    {
      auto pair = bit::core::make_compressed_pair(empty_non_final<0>{},non_empty_final<0>{});

      REQUIRE( sizeof(pair) == sizeof(non_empty_final<0>) );
    }
    SECTION("T1 is non-empty, final & T2 is empty, non-final")
    {
      auto pair = bit::core::make_compressed_pair(non_empty_final<0>{},empty_non_final<0>{});
