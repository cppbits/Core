...
TEST_CASE("bit::core::on_scope_exit( Fn&& fn )", "[raii]")
{
  ...
  {
    {
      auto scope = bit::core::on_scope_exit([&]()
      {
        flag = true;
      });
    }
...
TEST_CASE("bit::core::on_scope_error_exit( Fn&& fn )", "[raii]")
{
  bool flag = false;
  ...
  {
    {
      auto scope = bit::core::on_scope_error_exit([&]()
      {
        flag = true;
      });
    }
...
TEST_CASE("bit::core::on_scope_clean_exit( Fn&& fn )", "[raii]")
{
  bool flag = false;
  ...
  {
    {
      auto scope = bit::core::on_scope_clean_exit([&]()
      {
        flag = true;
      });
    }
