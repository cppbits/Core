...
static_assert( !std::is_trivially_destructible<bit::core::variant<std::string,bool>>::value,
...
{
  using variant_type = bit::core::variant<std::string,int>;
  ...
  SECTION("Default-constructs first alternative")
  {
    ...
    {
      REQUIRE( bit::core::get<0>( variant ) == std::string() );
    }
  }
}
...
{
  using variant_type = bit::core::variant<int,std::string>;
  ...
  SECTION("Copies the currently active element")
  {
    ...
    {
      REQUIRE( bit::core::get<1>( original ) == bit::core::get<1>( copy ) );
    }
  }
}
...
{
  using variant_type = bit::core::variant<int,std::unique_ptr<int>>;
  using namespace std::string_literals;

  auto original = variant_type{std::make_unique<int>(5)};
  auto pointer  = bit::core::get<1>( original ).get();
  ...
  SECTION("Copies the currently active element")
  {
    ...
    {
      REQUIRE( bit::core::get<1>( moved ).get() == pointer );
    }
    SECTION("Original content is moved")
    {
      REQUIRE( bit::core::get<1>( original ) == nullptr );
    }
  }
...
{
  using namespace std::string_literals;
  using variant_type = bit::core::variant<bool,std::string>;
  ...
  SECTION("Constructs based on function-overload resolution disambiguation")
  {
    SECTION("Resolves 'bool'-overload with C-string literal")
    {
      ...
      {
        REQUIRE( bit::core::get<0>( variant ) == true );
      }
    }
    SECTION("Resolves 'bool'-overload with boolean literal")
    {
      ...
      {
        REQUIRE( bit::core::get<0>( variant ) == false );
      }
    }
    SECTION("Resolves 'string'-overload with string literal")
    {
      ...
      {
        REQUIRE( bit::core::get<1>( variant ) == expected );
      }
    }
  }
}
...
TEST_CASE("variant::variant( in_place_type_t<T>, Args&&... )", "[ctor]")
{
  ...
  {
    auto variant = variant_type{ bit::core::in_place_type<std::string>, "hello world" };
    ...
    {
      REQUIRE( bit::core::get<1>( variant ) == "hello world"s );
    }
  }
}
...
TEST_CASE("variant::variant( in_place_type_t<T>, std::initializer_list<U>, Args&&...", "[ctor]")
{
  ...
  SECTION("Selects the overload based on type T")
  {
    auto variant = variant_type{ bit::core::in_place_type<std::string>,
                                 {'h','e','l','l','o',' ','w','o','r','l','d'},
                                 std::allocator<char>{} };
...
TEST_CASE("variant::variant( in_place_index_t<T>, Args&&... )", "[ctor]")
{
  ...
  SECTION("Selects the overload based on type T")
  {
    auto variant = variant_type{ bit::core::in_place_index<1>, "hello world" };
...
TEST_CASE("variant::variant( in_place_index_t<I>, std::initializer_list<U>, Args&&...", "[ctor]")
{
  ...
  SECTION("Selects the overload based on type T")
  {
    auto variant = variant_type{ bit::core::in_place_index<1>,
                                 {'h','e','l','l','o',' ','w','o','r','l','d'},
                                 std::allocator<char>{} };
...
{
  using namespace std::string_literals;
  using variant_type = bit::core::variant<bool,std::string,::throw_on_copy>;
  ...
  SECTION("Calls assignment operator if assigning active index")
  {
    ...
    SECTION("Changes value")
    {
      REQUIRE( bit::core::get<1>( variant ) == new_value );
    }
  }
  SECTION("Holds old value if copy throws")
  {
    ...
    {
      REQUIRE( bit::core::get<1>( variant ) == old_value );
    }
  }
}
...
{
  using namespace std::string_literals;
  using variant_type = bit::core::variant<bool,std::string,::throw_on_move>;
  ...
  SECTION("Becomes valueless by exception if move throws")
  {
    ...
    {
      REQUIRE( bit::core::variant_npos == variant.index() );
    }
...
TEST_CASE("variant::emplace<T>( Args&&... )", "[modifiers]")
{
  ...
  SECTION("Becomes valueless by exception if exception is thrown")
  {
    ...
    {
      REQUIRE( variant.index() == bit::core::variant_npos );
    }
...
{
  using namespace std::string_literals;
  using variant_type = bit::core::variant<std::string,bool,::throw_on_move>;
  ...
  SECTION("Swapping two variants with same alternative")
  {
    ...
    SECTION("Both variants hold types")
    {
      ...
      {
        REQUIRE( bit::core::get<0>(lhs) == rexpected );
      }
      SECTION("Right contains left's old value")
      {
        REQUIRE( bit::core::get<0>(rhs) == lexpected );
      }
    }
  }
  SECTION("Swapping two variants with different alternatives")
  {
    ...
    SECTION("Both variants hold types")
    {
      ...
      SECTION("Right contains left's old value")
      {
        REQUIRE( bit::core::get<1>(rhs) == lexpected );
      }
