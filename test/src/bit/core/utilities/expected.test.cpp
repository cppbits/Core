...
{
  auto expect = bit::core::expected<int>();
...
TEST_CASE("expected::expected( const expected& )")
{
  ...
  {
    auto expect = bit::core::expected<int>(5);
    auto copy = expect;
  ...
  {
    auto unexpect = bit::core::make_unexpected<std::error_condition>( std::errc::address_family_not_supported );
    auto expect = bit::core::expected<int>( unexpect );
    auto copy = expect;
